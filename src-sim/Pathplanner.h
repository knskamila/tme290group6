...
{
public:
    Pathplanner(std::string, float xBase, float yBase, int width, float dx, float xBegin, float yBegin, float xGoal, float yGoal) noexcept;
...
private:
    ...
    void createGraph(std::string filename, float xbase, float ybase, int width, float dx);
